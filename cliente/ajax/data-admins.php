...
class DataTableController
{
    ...
    public function data()
    {
        if (!empty($_POST)) {
            ...
            $url = "srja_caja?select=cod_caja&between1=".$_GET["between1"]."&between2=".$_GET["between2"]."&linkTo=fec_actualiza";
  
            
            
   
            // &startAt=0&endAt=1&orderAt=cod_empresa
            ...
            $dataJson = '{
                ...
                foreach ($data as $key => $value) {
                    ...
                    if($_GET["text"] == "flat"){
                        ...
                        $actions = "<a href='admins/edit/".base64_encode($value->cod_caja."~".$_GET["token"])."' class='btn btn-warning btn-sm mr-2'>

                        <i class='fas fa-pencil-alt'></i>

                        </a> 
                        
                        <a class='btn btn-danger btn-sm rounded-circle removeItem' idItem=".base64_encode($value->cod_caja."~".$_GET["token"])." table='srja_caja' page='admins' cod_empresa='".base64_encode($value->cod_empresa)."'>
